...
public class FastScrollRecyclerView extends RecyclerView implements RecyclerView.OnItemTouchListener {
  ...
  public FastScrollRecyclerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    try {
      mFastScrollEnabled = typedArray.getBoolean(R.styleable.FastScrollRecyclerView_fastScrollThumbEnabled,
          true);
    } finally {
      typedArray.recycle();
    }
  ...
  private int findMeasureAdapterFirstVisiblePosition(int passedHeight) {
    if (getAdapter() instanceof MeasurableAdapter) {
      ...
      for (int i = 0; i < getAdapter().getItemCount(); i++) {
        ...
        int bottom = top + measurableAdapter.getViewTypeHeight(this,
      ...
      int height = calculateScrollDistanceToPosition(getAdapter().getItemCount() - 1)
          + measurableAdapter.getViewTypeHeight(this,
          findViewHolderForAdapterPosition(getAdapter().getItemCount() - 1),
          getAdapter().getItemViewType(getAdapter().getItemCount() - 1));
      throw new IllegalStateException(String.format(
          "Invalid passed height: %d, [low: %d, height: %d]",
          passedHeight,
          low,
          height));
    } else {
      throw new IllegalStateException(
          "findMeasureAdapterFirstVisiblePosition() should only be called where the RecyclerView.Adapter is an instance of MeasurableAdapter");
    }
  ...
  private float findItemPosition(float touchFraction) {
    ...
    if (getAdapter() instanceof MeasurableAdapter) {
      ...
      for (int i = 0; i < getAdapter().getItemCount(); i++) {
        int top = calculateScrollDistanceToPosition(i);
        int bottom = top + measurer.getViewTypeHeight(this,
            findViewHolderForAdapterPosition(i),
            getAdapter().getItemViewType(i));
        if (i == getAdapter().getItemCount() - 1) {
  ...
  private void getCurScrollState(ScrollPositionState stateOut) {
    ...
    if (getAdapter() instanceof MeasurableAdapter) {
      ...
      stateOut.rowHeight = ((MeasurableAdapter) getAdapter()).getViewTypeHeight(this,
          findViewHolderForAdapterPosition(stateOut.rowIndex),
          getAdapter().getItemViewType(stateOut.rowIndex));
    } else {
      stateOut.rowTopOffset = getLayoutManager().getDecoratedTop(child);
  ...
  private int calculateScrollDistanceToPosition(int adapterIndex) {
    if (!(getAdapter() instanceof MeasurableAdapter)) {
      throw new IllegalStateException(
          "calculateScrollDistanceToPosition() should only be called where the RecyclerView.Adapter is an instance of MeasurableAdapter");
    }
    ...
    for (int i = 0; i < adapterIndex; i++) {
      ...
      totalHeight += measurer.getViewTypeHeight(this,
          findViewHolderForAdapterPosition(i),
          viewType);
    }
  ...
  private int calculateAdapterHeight() {
    if (!(getAdapter() instanceof MeasurableAdapter)) {
      throw new IllegalStateException(
          "calculateAdapterHeight() should only be called where the RecyclerView.Adapter is an instance of MeasurableAdapter");
    }
